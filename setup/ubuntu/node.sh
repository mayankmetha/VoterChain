...
then
    ...
    sudo add-apt-repository -y -r ppa:chris-lea/node.js > /dev/null 2>&1
    sudo rm -f /etc/apt/sources.list.d/chris-lea-node_js-*.list > /dev/null 2>&1
    sudo rm -f /etc/apt/sources.list.d/chris-lea-node_js-*.list.save > /dev/null 2>&1
    wget --quiet -O - https://deb.nodesource.com/gpgkey/nodesource.gpg.key | sudo apt-key add - > /dev/null 2>&1
    VERSION=node_10.x
    DISTRO="$(lsb_release -s -c)"
    echo "deb https://deb.nodesource.com/$VERSION $DISTRO main" | sudo tee /etc/apt/sources.list.d/nodesource.list > /dev/null 2>&1
    echo "deb-src https://deb.nodesource.com/$VERSION $DISTRO main" | sudo tee -a /etc/apt/sources.list.d/nodesource.list > /dev/null 2>&1
    sudo apt-get update > /dev/null 2>&1
